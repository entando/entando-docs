...
```
To check that the MS is working, go to `http://localhost:8081/`. The `run` command works only in a local environment and uses port 8081 for microservices as a rule.
