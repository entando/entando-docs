...
- `Bundle`: A Bundle is the deployment unit within an Entando Application. A Bundle can contain one or multiple components such as micro frontends, microservices, or any of the [component types](../../docs/curate/local-hub-bundle-details.md) available in Entando. 
...
Notes:
- A private repository can be used for a Bundle, but this requires [an additional Kubernetes secret](../curate/local-hub-private-git-repo.md#overview) before deployment via the App Builder.
