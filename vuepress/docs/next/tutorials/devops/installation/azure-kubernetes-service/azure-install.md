...
11. Keep the `Scale Method` set to `Autoscale` and the `Node count range` set to `1` to `5`
...
```
10. Access your application on the URL for the corresponding ingress, e.g. `http://EXTERNAL-IP.nip.io/entando-de-app/`
...
Double check your availability zones. By default an Azure cluster will include nodes from multiple zones but it will not provision storage for all of those nodes so you can't deploy your application.
