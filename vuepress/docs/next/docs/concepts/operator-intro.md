...
The goal of the operator is to provide automation and a set of default configuration to simplify and accelerate the deployment of an Entando application.
...
- This Secret is expected to have a private key, and a certificate for the hostname that the service
will be exposed on.
...
- If an EntandoDatabaseService has been deployed in the components namespace, and the DBMS specified on this EntandoDatabaseService
...
- For an EntandoApp Three schemas/database will be created: the Entando Port DB, tne Entando Serv DB and a database for
 the Entando Component Manager.
   - If the Port and Serv schemas/databases are empty, the Entando Operator  will use the underlying Entando App to populate these datbases with the data backup available in thestandard backup path in the WAR deployment.
