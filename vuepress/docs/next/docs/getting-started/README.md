...
Now that you've determined YOUR-HOST-NAME, you need to configure the Entando Application with it.
...
database connection or you can modify the `dbms` setting in the `entando-app.yaml`. 
:::

::: tip Entando Operator
The behavior of the Entando Operator is configurable using an optional ConfigMap named `entando-operator-config`. For example, on a slower network, you may want to increase the download timeouts. See [this page](../../tutorials/devops/entando-operator.md) for more information. 
:::
