...
[WARNING]
====
Building the MApp Engine from the source code is is a very specific, very technical use case for heavily customizing your Entando project. As such, we recommend that all but the most advanced projects use the standard process outlined beginning with the Creating an Entando Project section above.
====
